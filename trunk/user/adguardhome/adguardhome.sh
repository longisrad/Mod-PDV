...
# disable=1: tắt hẳn → khôi phục port 53
# disable=0: restart tạm → không cần khôi phục
agh_teardown_dns() {
    local disable=${1:-0}
    ...
    if [ "$AGH_DNS_PORT" != "53" ]; then
        ...
        if [ "$disable" = "1" ]; then
            # Tắt hẳn: set agh_enable=0 trước để services_ex.c
            # generate lại dnsmasq.conf với port=53
            nvram set agh_enable=0
            restart_dns 2>/dev/null || killall dnsmasq 2>/dev/null
            sleep 2
            logger -t "adguardhome" "dnsmasq restarted with port 53"
        fi
        # Restart tạm: không làm gì, AGH start lại ngay sau
    fi
}

# Kill AGH không teardown DNS (dùng cho restart)
agh_kill() {
    if [ -f "$AGH_PID" ]; then
...
agh_stop() {
    ...
    agh_kill
    agh_teardown_dns 1  # disable hẳn → khôi phục port 53
    logger -t "adguardhome" "Stopped"
}

agh_restart() {
    logger -t "adguardhome" "Restarting AdGuard Home..."
    agh_kill            # kill không teardown DNS
    sleep 1
    agh_start           # start lại ngay
}
